...
img = imread("Data for HW4\upload_dataset\2b.jpg");
img_size = size(img);
imgs = zeros([img_size(1)*img_size(2),171]);
idx = 1;
for i = 2: 200
    img_name = "Data for HW4\upload_dataset\"+int2str(i)+"b.jpg";
    if isfile(img_name)
...
Psi = sum(trains,2)/size(trains,2);
% Normalization
A_trains = trains-Psi;
A_valids = valids - Psi;
...
title('The Singular Value of the Smiling Expression Training Data');
...
sgtitle(['The Most 10 Representative Eigenfaces in ' ...
    'Training Set of Smiling Expression']);
%%
% Randomly pick a normalized image from tranining set of smiling expression
test_id = randi(100);
...
xlabel('Number of PCs')
grid on;
%% From the plot of MSE vs # of PCs, I choose #PCs = 91 to reconstruct the image
[img_valid_re,~,err_val] = W_N(91,eig_faces,img_size,img_valid);
figure
subplot(1,2,1)
...
colormap(gray(255));
title(["Reconstructed Image using 91 max eigenfaces", ...
    "error = " + err_val]); 
%%
